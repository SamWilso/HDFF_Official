...
class BasicBlock(nn.Module):
	def __init__(self, in_planes, out_planes, stride, dropRate=0.0):
		...
		self.ident = (self.equalInOut) and nn.Identity() or None
	def forward(self, x):
		...
		out = self.conv2(out)
		t = self.ident(x) if self.equalInOut else self.convShortcut(x)
...
class WideResNet(nn.Module):
	def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0):
		...
		self.fc = nn.Linear(nChannels[3], num_classes, bias=True) # CHANGE THIS BACK FOR TRAINING
	...
	## Forward function for 1D sub model
	def forward(self, x):
		out = self.conv1(x)
		out = self.block1(out)
		out = self.block2(out)
		...
		out = torch.abs(self.fc(out_feat))
		return out, out_feat
